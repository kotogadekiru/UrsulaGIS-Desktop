...
public class LSDetector extends Task<BufferedImage> {
	...
	private static final double DEFAULT_MIN_DENSITY = 0.25;//0.7
	private static final double DEFAULT_SCALE_FACTOR = 0.2;//0.8

	private static final int NTUPLE_LIST_SIZE = 7;
	 ...
	 * devuelvo angulos entre -PI y PI
	 */
	double angle_diff_signed(double a, double b) {
//		System.out.println("a= "+a+" b= "+b);
		//a -= b;
//		while (a <= -M_PI)//si es menor a -PI le sumo 2PI hasta que sea mayor a -PI
//			a += M_2__PI;
//		while (a > M_PI)
//			a -= M_2__PI;
		...
		//a= Math.abs(a);
		while(diff>M_2__PI){
//			System.out.println(" diff es > que PI diff="+diff);
			
			diff -= M_2__PI;// *(diff%M_PI);
...
//		if(diff<=M_PI && diff>=0 ){
//		//	System.out.println("angle_diff_signed OK " +a);
//		} else {
//			System.out.println("angle_diff_signed ERROR "+diff);
//		}
		return diff;
	}
	...
	double[] LineSegmentDetection(/* int[] n_out, */double[] img, int X, int Y,
			...
			int[] reg_x, int[] reg_y) {
		image_double image;
		ntuple_list out = new ntuple_list(NTUPLE_LIST_SIZE);
	...
	double[] lsd_scale_region(double[] img, int X, int Y, double scale
		...
		/* LSD parameters */
		double sigma_scale = DEFAULT_SIGMA_SCALE; 
		/*
		 * Sigma for Gaussian filter is computed as
		 * sigma = sigma_scale/scale.
		 */
		double quant =DEFAULT_MAX_NFA;
		/*
		 * Bound to the quantization error on the gradient
		 * norm.
		 */
		double ang_th = DEFAULT_GRADIENT_TOLERANCE; /* Gradient angle tolerance in degrees. */
		double log_eps = 0.0; /* Detection threshold: -log10(NFA) > log_eps */
		double density_th = DEFAULT_MIN_DENSITY; 
		/*
	...
	protected BufferedImage call() throws Exception {
		...
		try {
			BufferedImage myPicture = sourceImage;// ImageIO.read(new File("piet.jpg"));
			//		 Graphics2D g2d = myPicture.createGraphics();
			int x = myPicture.getWidth();
			int y = myPicture.getHeight();

			int bands = myPicture.getData().getNumBands();

			double [] arr = myPicture.getData().getPixels(0,0,x,y,new double[x*y*bands] );//new double[x*y*3]);
			...
			LSDetector lsd = new LSDetector();

			double [] out = lsd.lsd(arr2,x,y);
			//	edgesImage = writeEdges(out,x,y);
			edgesImage = new BufferedImage(x, y, BufferedImage.TYPE_INT_ARGB);
			Graphics2D g2d	=edgesImage.createGraphics();
...
//				}
			...
			for ( Line l : lines) {
				g2d.drawLine((int)l.getStartX(),(int)l.getStartY(),(int)l.getEndX(),(int)l.getEndY());
			}
			//
	...
	private BufferedImage writeEdges(double pixels[],int width,int height) {
		...
		for(int i =0;i<pixels.length;i++){
			ints[i]=(int) Math.round(pixels[i]);// new Double().intValue();
		}
		BufferedImage	edgesImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);

		edgesImage.getWritableTile(0, 0).setDataElements(0, 0, width, height, ints);
		return edgesImage;
	}
