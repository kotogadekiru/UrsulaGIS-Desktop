...
public class OrdenFertilizacionPaneController extends Dialog<OrdenFertilizacion>{
	...
	private static final String CONFIG_DIALOG_FXML = "OrdenFertilizacionPane.fxml"; //$NON-NLS-1$
	...
	private OrdenFertilizacion ordenFert;
	...
	public OrdenFertilizacionPaneController() {
		...
		this.setResultConverter(e -> {		
			if(ButtonType.OK.equals(e)){			
				return updateOrdenFertilizacion();
			}else{
				return null;
			}
		});
	...
	private OrdenFertilizacion updateOrdenFertilizacion() {
...
//		}
	...
	public void setLabor(OrdenFertilizacion l) {
		this.ordenFert = l;		
		try {
	...
	private void contructCaldoTable() {
		 List<OrdenFertilizacionItem> caldo = ordenFert.getItems();
	
		final ObservableList<OrdenFertilizacionItem> data =
				FXCollections.observableArrayList(
						caldo
						);
		SmartTableView<OrdenFertilizacionItem> table = new SmartTableView<OrdenFertilizacionItem>(data,
	...
	public static Optional<OrdenFertilizacion> config(OrdenFertilizacion labor2) {
		Optional<OrdenFertilizacion> ret = Optional.empty();
		
		try{
			FXMLLoader myLoader = new FXMLLoader(OrdenFertilizacionPaneController.class.getResource(
					CONFIG_DIALOG_FXML));
