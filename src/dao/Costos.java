...
//	private static final String CORRECCION_COSECHA = "CORRECCION_COSECHA";
	...
	//private Configuracion conf = Configuracion.getInstance();//TODO crear un objeto configuracion
...
//	public  final SimpleDoubleProperty correccionCosechaProperty= new SimpleDoubleProperty(getCorreccionCosecha());
	...
	private Costos(){
		...
		conf.modified.addListener((ov,object,nv)->{initProperties();});
	...
	private  Double getPrecioFertilizante(){
		return new Double(conf.getPropertyOrDefault(PRECIO_FERTILIZANTE,"0"));
	}
	...
	private  Double getCostoLaborFert(){
		return new Double(conf.getPropertyOrDefault(COSTO_LABOR_FERTILIZACION,"0"));
	}
	...
	private  Double getPrecioSemilla(){
		return new Double(conf.getPropertyOrDefault(PRECIO_SEMILLA,"0"));
	}
	...
	private  Double getCostoLaborSiembra(){
		return new Double(conf.getPropertyOrDefault(COSTO_LABOR_SIEMBRA,"0"));
	}
	...
	private  Double getCostoLaborPulverizacion(){
		return new Double(conf.getPropertyOrDefault(COSTO_LABOR_PULVERIZACION,"0"));
	}
	...
	private  Double getPrecioGrano(){
		return new Double(conf.getPropertyOrDefault(PRECIO_GRANO,"0"));
	}
	...
	//	}
	private  Double getCostoFijoHa(){
		return new Double(conf.getPropertyOrDefault(COSTO_FIJO_POR_HA,"0"));
	}

	private  Double getCorreccionCosecha(){
		return new Double(conf.getPropertyOrDefault(CORRECCION_COSECHA,"100"));
	}
