...
public abstract class ProcessMapTask extends Task<Quadtree>{
	...
	protected static Classifier clasifier=null;
	...
	protected Paint getColorFor(Double rinde) {				
...
//		double porcent = delta/rango;
		...
		if(histograma != null){
			...
			return  colors[getColorByJenks(rinde,clasifier)];
		}
	...
	public static int getColorByJenks(Double double1,Classifier classifier) {
		try{
	...
	public static Double[] constructHistogram(List<? extends Dao> elementosItem){
		...
		if(elementos.size()>colors.length){
			int elementsPerColour = elementos.size()/colors.length;
			for(int i = 0;i<colors.length;i++){	
			Dao element = elementos.get(i*elementsPerColour);//si elementos es menor a colors?
			histograma[i]=element.getAmount();	
			//System.out.println("nuevo limite del histograma para i="+i+" es "+ histograma[i]);
			}
