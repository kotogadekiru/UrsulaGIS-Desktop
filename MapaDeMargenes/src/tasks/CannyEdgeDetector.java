...
public class CannyEdgeDetector extends Task<BufferedImage> {
	...
	public void setRoofThreshold(float threshold){
		this.roofThreshold = threshold;
	}
	/**
	...
	public void process() {
		...
		readLuminance();
		updateProgress(20, 60);
		if (contrastNormalized) normalizeContrast();
		updateProgress(30, 60);
		computeGradients(gaussianKernelRadius, gaussianKernelWidth);
		updateProgress(30, 60);
		int low = Math.round(lowThreshold * MAGNITUDE_SCALE);
		int high = Math.round( highThreshold * MAGNITUDE_SCALE);
		int roof = Math.round( roofThreshold * MAGNITUDE_SCALE);
		performHysteresis(low, high, roof);
...
// 	}
	...
	private void performHysteresis(int low, int high, int roof) {//version2
		//NOTE: this implementation reuses the data array to store both
		//luminance data from the image, and edge intensity from the processing.
		//This is done for memory efficiency, other implementations may wish
		//to separate these functions.
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (data[offset] == 0 && magnitude[offset] >= high && magnitude[offset] <= roof) {
					follow(x, y, offset, low);
				}
				offset++;
			}
	...
	private void readLuminance() {
		...
		if (type == BufferedImage.TYPE_INT_RGB || type == BufferedImage.TYPE_INT_ARGB) {
		 ...
		 System.out.println("supported image type: "+type);
	}
